...
class Experiment:
    ...
    def plan_double_arm(
        ...
        # Reset planners per segment (prevents cross-contamination between calls)
        update_environment(env, LocationType.RIGHT, env_update1, cubes_real)
        self._reset_planner_for_segment(planner_right)
        path_right, cost_right = planner_right.find_path(start_conf=right_arm_start, goal_conf=goal_right)


        update_environment(env, LocationType.LEFT, env_update2, cubes_real)
        self._reset_planner_for_segment(planner_left)

        path_left, cost_left = planner_left.find_path(start_conf=left_arm_start, goal_conf=goal_left)

        if path_left is None:
            raise RuntimeError(f"Planner failed: no path found for segment '{description}' (active=LEFT).")
        if path_right is None:
            raise RuntimeError(f"Planner failed: no path found for segment '{description}' (active=RIGHT).")

        # create the arm plan
        self.push_step_info_into_single_cube_passing_data(
            description,
            [LocationType.LEFT, LocationType.RIGHT],
            "moveboth",
            [right_arm_start.tolist(), left_arm_start.tolist()],
            [[p.tolist() for p in path_right], [p.tolist() for p in path_left]],
    ...
    def plan_single_cube_passing(
        ...
        description = "right_arm => [cube pickup -> meeting], left_arm => [start -> meeting]"
        log(msg=description)
        self.plan_double_arm(
            planner_left=planner_left, 
            planner_right=planner_right,
            left_arm_start=left_arm_start,
            right_arm_start=cube_approach,
            goal_left=self.left_arm_meeting_conf,
            goal_right=self.right_arm_meeting_conf,
            description=description,
            env_update1=left_arm_start,
            env_update2=right_at_meeting,
            cubes_real=cubes_wo_i,
            env=env,
            )
