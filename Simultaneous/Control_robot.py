...
class BaseRobot:
    ...
    def move_path(self, path, asynchronous = False):
        if asynchronous:
            self.move_path_sync(path[0], path[1], asynchronous=asynchronous)
            return
        speed = 0.5
        acceleration = 0.5
        blend_radius = 0.05
    ...
    def move_path_sync(self, path1, path2, asynchronous = True):
        speed = 0.5
        acceleration = 0.5
        blend_radius = 0.05
        path_with_params1 = [
            [*target_config, speed, acceleration, blend_radius] for target_config in path1
        ]
        path_with_params2 = [
            [*target_config, speed, acceleration, blend_radius] for target_config in path2
        ]
        path_with_params1[-1][-1] = 0 # reach the end point
        path_with_params2[-1][-1] = 0 # reach the end point
        current_config1 = np.array(self.robot_left.getActualQ())
        current_config2 = np.array(self.robot_right.getActualQ())
        goal_config1 = np.array(path1[-1])
        goal_config2 = np.array(path2[-1])

        # np.max(np.abs(current_config1 - goal_config1)) > 0.1

        self.robot_left.moveJ(path_with_params1, asynchronous=asynchronous)
        self.robot_right.moveJ(path_with_params2, asynchronous=asynchronous)
        current_config1 = np.array(self.robot_left.getActualQ())
        current_config2 = np.array(self.robot_right.getActualQ())
        goal_config1 = np.array(path1[-1])
        goal_config2 = np.array(path2[-1])

        if np.max(np.abs(current_config1-goal_config1)) > 0.1:
            self.robot_left.moveJ(goal_config1, asynchronous=asynchronous)
        if np.max(np.abs(current_config2-goal_config2)) > 0.1:
            self.robot_right.moveJ(goal_config2, asynchronous=asynchronous)
