...
from Simultaneous.Control_robot import BaseRobot
    ...
    with open(json_path, 'r') as openfile:
            ...
            for step in steps:
                ...
                for i in range(len(step["active_id"])):
                    ...
                    if step["command"][i] != "moveboth":
                        arm_id = step["active_id"][i]
                    else:
                        arm_id=step["active_id"][0]
                    ...
                    if step["command"][i] == "moveboth":
                        # robot.move([curr_conf, conf])
                        robot.move_path(step["path"][i],True) # Default is false, this means the function will block until the movement has completed.
